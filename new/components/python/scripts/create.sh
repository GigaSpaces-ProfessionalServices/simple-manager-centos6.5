...
export INSTALL_PYTHON=$(ctx node properties install_python)
...
function install_python
{
	...
	sudo yum update -y
    sudo yum install -y zlib-devel bzip2-devel openssl-devel xz-libs wget gcc g++ ncurses-devel sqlite-devel readline-devel tk-devel
    sudo mkdir /py27
    cd /py27
    sudo wget http://www.python.org/ftp/python/2.7.10/Python-2.7.10.tar.xz
    ...
    # Install the right pip and virtualenv for this python version
    sudo wget https://bitbucket.org/pypa/setuptools/raw/bootstrap/ez_setup.py
    sudo /usr/bin/python2.7 ez_setup.py
    sudo /usr/bin/easy_install-2.7 pip virtualenv

	echo "alias python=/usr/bin/python2.7" | sudo tee -a /root/.bashrc
	echo "alias python=/usr/bin/python2.7" | sudo tee -a ~/.bashrc
...
if [ "${INSTALL_PYTHON}" = "true" ]; then
    ctx logger info "Installing Python 2.7.10... this may take some time.."
